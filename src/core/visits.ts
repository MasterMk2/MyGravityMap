...
/**
 * 隙間の前後で許容する移動距離（メートル）。DESIGN.md §4.1-3 の R。
 *
 ...
 *
 * 値は実測で決めた（DESIGN.md 未決事項 D / `npm run validate:visits`）。
 * R を 1 段緩めて増える復元のうち「当たり」が「外れ」を上回るのは 175m までで、
 * 200m から先は割に合わなくなる。120m から 175m までは横ばい（同数ずつ増える）で、
 * その範囲の上端を取ってある。100m より内側は締めすぎで、拾えるはずの滞在を捨てる。
 */
const DEFAULT_MAX_MOVE_METERS = 175
