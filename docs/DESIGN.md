...
- **D. 滞在復元のパラメータ** — 検証方法は実装した（`npm run validate:visits`、
  ...
  | R | 復元件数 | 再現率 | 適合率 | 位置誤差 中央 | 同 p75 |
  |---|---|---|---|---|---|
  | 60m | 654 | 15% | 55% | 40m | 1,435m |
  | 120m（現在の既定） | 727 | 18% | 56% | 43m | 1,435m |
  | 250m | 801 | 20% | 56% | 60m | 1,201m |
  | 500m | 866 | 23% | 55% | 76m | 1,099m |
  | 1000m | 916 | 27% | 53% | 103m | 1,053m |
  | 距離判定なし | 1,370 | 85% | 44% | **1,224m** | 4,232m |

  分かったこと:
  - **T は効かない。** 軌跡を 30 分以上の空白で切っている以上、隙間は必ず 30 分以上ある。
    T を 30 分より短くしても結果は 1 件も変わらない（設計時の T = 25 分は到達できない値）。
    60 分に上げると件数が 4 割減るが、再現率も適合率もほぼ動かない。効く軸は R だけ。
  - **R に鋭い最適値は無い。** 上げるほど再現率が上がり位置誤差が悪化する、なだらかな
    トレードオフ。適合率は 120–250m あたりでわずかに山になる。
  - **距離判定を外すと再現率 85% まで跳ねるが、位置誤差の中央値が 1.2km になる。**
    「その時間そこに居た」ことにはなっても、場所としては別物。
