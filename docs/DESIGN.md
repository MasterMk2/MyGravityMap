...
- **D. 滞在復元のパラメータ（決着）** — 検証は実装して回した（`npm run validate:visits`、
  `tests/validate-visits.harness.ts`）。結論は **R = 120m 据え置き、T は効かないので触らない**。
  ...
  **最適値の判定基準**: R を 1 段緩めると復元件数が増える。その増えた分のうち
  ...
  | **175m（限界比 1.0 の上限）** | 726 | 401 | 324 | 1.05 | 59m | 1.87 |
  ...
  **時間だけで測った再現率は単独では読めない**（隙間が長ければ中の短い滞在はまとめて覆われる）。
  ...
  分かったこと:
  - **R の最適値は 175m。ただし 120m から 175m までは限界比がほぼ 1.0**、
    つまり緩めても当たりと外れが同数ずつ増えるだけで、実質は横ばい。
    **現在の既定 120m はこの平らな範囲の下端にある。** 動かす強い理由は無い。
  - **効く境界は 200m。** そこから先は増える復元の 2/3 が外れになる。
    120m か 175m かは好みの範囲だが、200m を超えるのは損。
