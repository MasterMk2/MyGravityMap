...
`TripsLayer` はタイムスタンプを float32 の頂点属性として扱う。float32 の相対精度は 2⁻²⁴ なので、
Unix 秒（約 1.79 × 10⁹）では**刻みが約 107 秒**になる。点間隔の中央値は 240 秒だから、
半数近くの点が隣とほぼ同じ値に潰れ、尾が 107 秒刻みでガタつき短いトリップは消える。
