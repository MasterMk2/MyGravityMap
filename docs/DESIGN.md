...
- **D. 滞在復元のパラメータ** — 検証は実装して回した（`npm run validate:visits`、
  `tests/validate-visits.harness.ts`）。T については決着（効かないので触らない）。
  R は 120m と 175m のどちらを既定にするかが残っている（下記の通り実質は横ばい）。
  ...
  **最適値の判定基準**: R を 1 段緩めると復元件数が増える。その増えた分のうち
  ...
  | **120m（現在の既定）** | 698 | 387 | 311 | 1.01 | 50m | 2.05 |
  | 150m | 713 | 395 | 318 | 1.07 | 54m | 1.98 |
  | **175m（限界比 1.0 を保つ最後の段）** | 726 | 401 | 324 | 1.05 | 59m | 1.87 |
  ...
  **時間だけで測った再現率は単独では読めない**（隙間が長ければ中の短い滞在はまとめて覆われる）。
  ...
  - **120m から 175m までは限界比がほぼ 1.0**（1.01 / 1.07 / 1.05）。緩めても当たりと
    外れが同数ずつ増えるだけで、実質は横ばい。**現在の既定 120m はこの平らな範囲の下端。**
    175m にすると当たりが 387 → 401 件に増えるが、外れも 311 → 324 件に増える。
