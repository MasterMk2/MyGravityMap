...
6. **トリップ分割**: 全点を時刻順に並べて重複を除き、30 分以上の空白で切って `Trip` を作る（§6.4）。
   200 km/h 超の区間を含むトリップに `isFlight` を立てる。`activity` と時間で突き合わせて交通手段を付ける。
...
2. **200 km/h 超の 61 区間は飛行機**（900 km/h 超は 0 なので誤差ではない）。
   ノイズとして捨てず、**大圏コースの弧＋点線**で別表現にする。
3. **2019 年の再生はなめらかにならない**（75 分に 1 点）。これはデータの限界なので、
...
- **補間**: 線形（既定）/ 補間なし（点から点へ跳ぶ）/ 飛行機区間のみ大圏補間。
...
type Trip = {
  path: Float32Array;      // [lon, lat, lon, lat, ...]
  timestamps: Int32Array;  // Unix 秒（点数は path と一致）
  mode: Move['mode'];      // activity セグメントと時間で突き合わせて推定
  year: number;
  isFlight: boolean;       // 200 km/h 超の区間を含む
};
```

- 全期間でも重複除去後 115,027 点 ≒ 約 1.4 MB。GPU に一度上げてしまえば、
...
| **P1a 再生** | 期間選択 + **タイムライン再生**（グラデーション / 実線の切替、速度プリセット、空白スキップ、滞在円）。トリップ分割込み。**まずこれが動くと一番うれしい** |
...
- **D. 滞在復元のパラメータ** — R = 120 m / T = 25 分は 2 時間バケットの粗さからの初期値。
  検証は「2024 年以降で復元結果と `visit` を突き合わせる」だけでは**不十分**。
  2024 年以降は記録が濃い（15 h/日）ため、そこで最適化した値は 2019 年（6 h/日）には転用できない。
  正しくは **2024–2025 の `timelinePath` を 2019 年の密度まで間引いてから復元し、
  同じ期間の Google `visit` と突き合わせる**。これで「2019 年の密度で何が復元できるか」が測れる。
  評価指標は再現率（visit のうち復元できた割合）と位置誤差の中央値（P1 で実施）。
