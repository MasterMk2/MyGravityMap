...
6. **カバレッジ算出**: 年ごとに `YearCoverage` を計算する（§1.2.1 の表を実データから毎回作る）。
   正規化・時間モードの可否判定・UI の警告表示がすべてこれを参照する。
7. **保存**: IndexedDB へ。ファイルのハッシュをキーにして、同じファイルなら再解析しない。
...
| **P1 中核** | 重力マップ / ランキング / 軌跡 の 3 ビュー + 期間フィルタ + データ品質バンド。derived 滞在復元込み。**この時点で「8 年分が頻度モードで見える」**（時間モードは 2024 年 10 月以降のみ） |
