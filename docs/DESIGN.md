...
3. **滞在の復元**（2018–2023 対策）:
   ...
   （**当初の設計値**は R = 120 m、T = 25 分。2 時間バケットの粗さから置いた値で、
   UI で調整可のつもりだった。実際に採用した値は下の実装メモを見ること）。
   ...
   - **現在の実装（`core/visits.ts`）は簡略版**。点列を直接クラスタリングせず、
     ...
     隙間の前後の点の距離が R 以内なら滞在とみなす（T の既定は 1 時間、R = 175 m）。
     R は「留まっていた」と「移動中に記録が落ちた」を分けるために効く。
     T / R の UI 露出はまだ無い。R の値は実測で決めた（§8 の D）。
     T は分割閾値（30 分）より短い隙間が存在しないので効かない。
...
### 決定済み（2026-08-27・実データの検証で決定）

- **D. 滞在復元のパラメータ → R = 175 m / T は触らない で確定。**
  検証を実装して回した（`npm run validate:visits`、`tests/validate-visits.harness.ts`）。
  ...
  間引きは乱数なので、どのバケットが落ちるかで隙間の出方が変わる。1 本だけだとその形に
  過適合するので、シードを変えて 25 本まわした平均で見る。

  **最適値の判定基準**: R を 1 段緩めると復元件数が増える。その増えた分のうち
  「当たり（位置まで合っている）」が「外れ」の何倍あるかを**限界比**として見て、
  1.0 を割る手前を上限とする。F1 は使えない — R を緩めるほど単調に上がり、
  距離判定なしが常に勝ってしまう（再現率の増分が適合率の減分を上回るため）。

  | R | 復元件数 | 当たり | 外れ | 限界比 | 位置誤差 中央 | 滞在時間比 |
  |---|---|---|---|---|---|---|
  | 60m | 633 | 348 | 285 | 2.44 | 41m | 2.54 |
  | 100m | 676 | 376 | 300 | 1.31 | 48m | 2.12 |
  | 120m（変更前の既定） | 698 | 387 | 311 | 1.01 | 50m | 2.05 |
  | 150m | 713 | 395 | 318 | 1.07 | 54m | 1.98 |
  | **175m（採用。限界比 1.0 を保つ最後の段）** | 726 | 401 | 324 | 1.05 | 59m | 1.87 |
  | 200m | 737 | 406 | 331 | **0.61** | 63m | 1.85 |
  | 250m | 761 | 417 | 344 | 0.91 | 76m | 1.78 |
  | 500m | 826 | 441 | 385 | 0.42 | 105m | 1.71 |
  | 1000m | 867 | 452 | 415 | 0.41 | 121m | 1.71 |
  | 距離判定なし | 1,283 | 564 | 720 | 0.34 | **1,352m** | 5.19 |

  交差点（175m → 200m）はシード 5 本・9 本・25 本のいずれでも同じ位置に出る。
  ただしシードを増やして消えるのは標本のばらつきだけで、刻み幅の粗さは残る。
  段ごとの限界比は 200m で 0.61、250m で 0.91、350m で 0.79 と上下するので、
  「175m がちょうど最適」ではなく「**境界はこの刻みでは 175m と 200m の間**」と読む。

  「外れ」は**当たりと確認できなかった**件数であって、確実な誤りの件数ではない。
  Google 側に正解が無い期間（記録も訪問データも無い時間帯）の復元もここに入る。
  絶対値ではなく、R を緩めたときの増え方を比べるための指標として使う。

  「吸収」（復元できた正解 ÷ それを担った derived の件数）はどの R でも 1.7〜2.3 ある。
  1 件の derived が平均 2 件前後の正解を覆って「復元した」ことになっているので、
  **時間だけで測った再現率は単独では読めない**（隙間が長ければ中の短い滞在はまとめて覆われる）。
  上の表で再現率ではなく件数を出しているのはそのため。

  分かったこと:
  - **120m から 175m までは限界比がほぼ 1.0**（1.01 / 1.07 / 1.05）。緩めても当たりと
    外れが同数ずつ増えるだけで、実質は横ばい。この平らな範囲のどこを取るかは好みで、
    カバレッジ寄りに上端の 175m を採った（当たり 387 → 401 件、外れ 311 → 324 件）。
  - R の結論は T に依存しない。実装の既定である T = 60 分でも同じ形になる
    （120m で 1.04、175m で 1.04、200m で 0.55）。
  - **200m 以降は限界比が 1.0 を超えない。** その段（175m → 200m）だけを見ると
    増えた 11 件のうち当たりは 4 件で、残りは当たりと確認できなかった分。
    どこまで緩めるかの上限としてはここが目安になる。
  - 100m より内側では限界比が 1.3〜2.4 あり、締めすぎ（拾えるはずの当たりを捨てている）。
  ...
  - **距離判定を外したときの「再現率 85%」は見かけ。** 位置誤差の中央値が 1.35km、
    滞在時間比の中央値が 5.19（正解の 5 倍の長さ）、吸収 2.36。
    実体は「長い隙間を丸ごと 1 件の滞在にしたので、その中の正解をまとめて覆った」だけで、
    復元 1,283 件のうち位置まで当たったのは 564 件、外れが 720 件。場所としては別物。
