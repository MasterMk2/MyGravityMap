...
- **D. 滞在復元のパラメータ** — R = 120 m / T = 25 分は 2 時間バケットの粗さからの初期値。
  ...
  - 実測（開発機の実データ・件数のみ）: 時間条件だけを満たす隙間は 3,517 件。
    そのうち隙間の前後の距離が 120m 以内は 1,585 件（45%）、250m 以内 2,170 件、
    500m 以内 2,686 件。距離の中央値は 175m。R をどこに置くかで復元件数が倍近く動くので、
    この値は検証で決めるべきパラメータであって、既定値のまま放置してよい類のものではない。
